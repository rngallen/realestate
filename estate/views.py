    ...
    if request.method == "POST":
        ...
        if request.user.is_authenticated:
            has_contacted = Contact.objects.filter(
                user_id=request.user.pk, house_id=house_id, email=email
            )
            if has_contacted:
                messages.error(
                    request, "You have already make inquiry for this house.!!"
                )
                ...
                try:
                    send_mail(
                        ...
                        message,
                        from_email,
                        [realtor_email, "ramadhan@maycom.co.tz"],
                    )
                except BadHeaderError:
                    messages.error(
                        request, f"Invalid Header Found, Kindly send again!!"
                    )
        else:
            inquiry = Contact(
                title=title,
